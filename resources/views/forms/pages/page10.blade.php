...
<style>
    ...
    #page10 input:disabled, #page10 textarea:disabled {
...
<div class="bg-white/90 backdrop-blur-md shadow rounded mb-8 p-4 px-8" id="page10">
    ...
    <div class="flex justify-end py-4">
        ...
        <button id="saveBtn10" class="bg-green-500 text-white px-4 py-2 rounded mr-2" style="display: none;" onclick="saveChanges10()">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline-block mr-1" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
        ...
        <button id="cancelBtn10" class="bg-gray-500 text-white px-4 py-2 rounded" style="display: none;" onclick="cancelEdit10()">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline-block mr-1" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                    ...
                    <tbody>
                        ...
                        @forelse($page10Data as $index => $parameter)
                            <tr>
                                ...
                                <td class="parameter-action-cell" style="display: none; text-align: center; vertical-align: middle;">
        ...
        function saveChanges10() {
            ...
            parameterRows.forEach((row, index) => {
                ...
                if (inputs.length >= 6 && textarea) {
                    ...
                    if (parameterName || subParameterA || subParameterB || subParameterC || subParameterD || subParameterE || achievementDescription) {
                        // Word count validation for achievement description
                        const wordCount = achievementDescription ? achievementDescription.split(/\s+/).filter(word => word.length > 0).length : 0;
                        if (wordCount > 50) {
                            showToast(`Achievement description in row ${index + 1} exceeds 50 words. Please reduce to 50 words or less.`, 'error');
                            return false;
                        }
